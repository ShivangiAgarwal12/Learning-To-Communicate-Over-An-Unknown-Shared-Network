...
class Vehicle_model():
   ...
   def __init__(self, v, px, py, vx, vy, age_hist=0):
      """
      State Parameters
      ----------
      v : car_id
      ...
      history_policy = history_packet + self.control_actions + 6
      ...
      self.reset_age_store = []
      self.reset_age_store.append(0)
   ...
   def env_step(self, agent_AC, k, model, eps_threshold, steps_done):
      ...
      if k == 0:
           ...
           self.age_store.append(self.age_policy[0].item())
                                        ...
                                        torch.tensor([[self.avg_a]]).to(device),\
                                            ...
                                            torch.tensor([[self.avg_backlog]]).to(device),\
                                               torch.tensor([[self.backlog[-1] ]]).to(device),\
                                                   torch.tensor([[self.avg_reset]]).to(device),\
           ...
           # pdb.set_trace()
           self.avg_backlog = self.ema_alpha * self.backlog[-1] + (1- self.ema_alpha)*self.avg_backlog
           ...
           # action_probs = action_probs.cpu().detach().numpy()
           p = p.cpu().numpy()[0]
           # p = np.random.choice(2,p=[1-config.q/2,config.q/2])
           self.lp.append(lp)
           # Append probability.
           self.last_p.append(action_probs[0,1].item())
      ...
      # Average query.
      self.avg_p = self.ema_alpha * p + (1- self.ema_alpha)*self.avg_p
      ...
      if packet_policy:
          ...
          self.avg_a = self.ema_alpha * self.age_policy[0].item() + (1- self.ema_alpha)*self.avg_a
          ...
          # Average reset.
          self.avg_reset = self.ema_alpha * temp + (1- self.ema_alpha)*self.avg_reset
          ...
          self.reset_age_store.append(self.reset_age)
          
          # Average reset.
          self.avg_reset = self.ema_alpha * self.reset_age + (1- self.ema_alpha)*self.avg_reset
