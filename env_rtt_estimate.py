...
input to LSTM-> estimate and RTT
corrected with estimate and RTT only to actor-critic architecture
"""
#%%import libraries
import numpy as np
import math
...
class Vehicle_model():
   ...
   def env_step(self, agent_AC, k, model,  steps_done):
      ...
      if k == 0:
           ...
           temp_policy = torch.cat((state_learn.T, torch.tensor([[self.rtt_policy[-1].item()]]).to(device)),0)
...
#         pdb.set_trace()
      ...
      if packet_received == True:
         ...
         with torch.no_grad():
             ...
             self.LSTM_input[k] = torch.cat((self.store_packet[-1],self.previous[-1],self.rtt_policy), 0)
      ...
      #packet recieved
         ...
         with torch.no_grad():
            ...
            self.LSTM_input[k] = torch.cat((packet, self.previous[-1],self.rtt_policy),0).requires_grad_(True)
