# -*- coding: utf-8 -*-
"""
lstm added with states from backlog, reset, estimates and received packets, same states as 
policy and critic network , actor network doesnt have any received packet, no history added
added with different buffers for different alphas 
"""
...
env = SVS.Vehicle_model(car_id,x_init,x_init, v_init,v_init, 0, 0)
#%% RL agent initialize.
...
agent_DSAC = RL.Dsac(env, replay_size = replay_size, batch_size = batch_size, gamma=gamma,\
               lr= 1.5*1e-4, n_steps=n_steps,age_hist=age_hist,lstm_hid=hidden_dim_service[0])
...
for ep in range(max(ep,0),episodes):
    ...
    choice_service = np.random.choice(range_1, 1, prob_range)[0]
    
    if choice_service == 0.05:
    ...
    if choice_service == 0.3 or choice_service == 0.5:
        config.q = np.random.uniform(choice_service, choice_service + 0.2)
        # agent_DSAC.t_alpha = 0.8
        # lower, upper, scale = choice_service, choice_service + 0.2, 1/4.0
        # q_sample_high = stats.truncexpon(b=(upper-lower)/scale, loc=lower, scale=scale)
        # config.q = q_sample_high.rvs(1)
        agent_DSAC.t_alpha = 0.7
        choice_buffer = 2
    ...
    env = SVS.Vehicle_model(car_id,x_init,x_init_y, v_init,v_init_y, c_init, c_init_y, age_hist=10)
    ...
    for t in range(t_eps-n_steps):
        ...
        if ep>=int_up and t%t_up==0:
            t_up = max(t_up-10,1)
        
        # if ep>=int_up and t%t_up==0 and flag_LSTM == 0:
        if ep>=int_up and t%t_up == 0 :
            ...
            # Recompute the obs and obs2 input.
            with torch.no_grad():
    ...
    if ep%500 == 0 and ep > 0:
      path = os.getcwd()
      env_name = 'Policy_MSE'
      dir_save = path + '/' +str(env_name) + '_DSAC_LSTM/Multiple_buffers/Age_loss/History_10/Ver2/Train_'  + str(ep) + '/' + \
...
path = os.getcwd()
env_name = 'Policy_MSE'
dir_save = path + '/' +str(env_name) + '_DSAC_LSTM/Multiple_buffers/Age_loss/History_10/Ver2/Train/'  + \
             str(agent_DSAC.t_alpha) + '_'  + str(n_steps)
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
model_DSAC = dir_save + '/Model_DSAC'
