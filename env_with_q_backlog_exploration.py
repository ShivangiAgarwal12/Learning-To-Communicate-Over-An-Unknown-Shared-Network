...
class Vehicle_model():
   ...
   def __init__(self, v, px, py, vx, vy, age_hist=0):
      ...
      self.rand_act = True
      # Rnadom max 
      self.q_rand = np.random.random(1)[0]/2.0
      # self.q_rand = config.q/2.0
   ...
   def env_step(self, agent_AC, k, model, eps_threshold, steps_done):
      ...
      if self.rand_act:
          p = np.random.choice(2,p=[1-self.q_rand,self.q_rand])
      ...
      #and prior
         ...
         with torch.no_grad():
            ...
            temp_lstm = torch.cat((packet, age[0:], self.previous[-1], self.control,\
                                        torch.tensor([[self.avg_reset]]).to(device),\
                                            torch.tensor([[self.reset_age]]).to(device)),0)
            
            self.LSTM_input[k] = torch.cat((packet, age[0:], self.previous[-1], self.control,\
                                        torch.tensor([[self.avg_reset]]).to(device),\
                                            torch.tensor([[self.reset_age]]).to(device)),0)
