'''
passing instantaneous age, action, backlog, inter arrival time through lstm
'''
import numpy as np
import torch
...
class MLPActor(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        self.lstm_actor = nn.LSTMCell(32, 4)
        self.relu = nn.ReLU()
    ...
    def forward(self, obs):
        ...
        # hx, cx = self.lstm_actor(obs[:,4:])
        # pdb.set_trace()
        # out = self.relu(self.fc(hx))
        # out = self.fc2(out)
        obs_new = torch.cat((obs[:,0:4], hx[:]),1)
