...
input to actor critic (not lstm)-> estimate, age, and RTT,backlog and inter-arrival time
input shape to actor-critic nn is [8*1], estimate (4), age(1), backlog(1),inter-arrival(1),rtt(1)
"""
#%%import libraries
import numpy as np
import math
...
class Vehicle_model():
   ...
   def __init__(self, v, px, py, vx, vy,cx, cy, age_hist = 0):
      ...
      #update with RTT
      history_policy = history_rep + history_age + self.control_actions
   ...
   def env_step(self, agent_AC, k, model,  steps_done):
      ...
      with torch.no_grad():
          ...
          # pdb.set_trace()
          temp_lstm = torch.cat((packet, self.age_policy,self.previous[-1]),0)
      ...
      if k == 0:
           ...
           # pdb.set_trace()
           # input to actor critic nn is estimate,age,backlog,inter-arrival time and rtt
           temp_policy = torch.cat((state_learn.T, torch.tensor([[self.rtt_policy[-1].item()]]).to(device), \
            torch.tensor(self.age_h).unsqueeze(0).T.to(device),\
...
#         pdb.set_trace()
      ...
      if packet_received == True:
         ...
         with torch.no_grad():
             ...
             self.LSTM_input[k] = torch.cat((self.store_packet[-1], age[0:],self.previous[-1]), 0)
      ...
      #packet recieved
         ...
         with torch.no_grad():
            ...
            temp_lstm = torch.cat((packet, age[0:],self.previous[-1]),0)
                
            self.LSTM_input[k] = torch.cat((packet, age[0:], self.previous[-1]),0).requires_grad_(True)
