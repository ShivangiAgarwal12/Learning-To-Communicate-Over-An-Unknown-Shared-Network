# -*- coding: utf-8 -*-
"""
main_env with RTT, cleaned code with RTT in input, with another input of estimate and rtt
"""
#%%
import math
import random
...
# import env_rtt_estimate as SVS
import env_rtt_estimate as SVS
...
# pdb.set_trace()
# DQN_CE agent.
#%%initialise the RL agent, general learning rate is 2.5
agent_DSAC = RL.Dsac(env, replay_size = replay_size, batch_size = batch_size, gamma=gamma,\
               lr= 3*1e-4, n_steps_agent=n_steps_agent,age_hist=age_hist,lstm_hid=hidden_dim_service[0])
...
state_str_t = np.zeros((episodes,t_eps+1,6))
#camera state
...
for ep in range(max(ep,0),episodes):
   #updating the flag for LSTM
    ...
    # pick_constant = np.random.choice([0.1,0.3], 1)[0]
    ...
    if choice_service == 0.1 or choice_service == 0.3:
        config.q = np.random.uniform(choice_service, choice_service + 0.1)
    ...
    for t in range(t_eps-n_steps):
        ...
        new_state_policy , MSE_input_batch, MSE_output, \
            reward, done, action, loga, action_probs =  env.env_step(agent_DSAC, t, \
                                                                     agent_lstm, \
                                                     steps_done)
        ...
        if t >=1:
            ...
            #range 1000
            rew_n_str.append(config.r_scale+config.r_scale*(reward.item()/80000.0))
        ...
        # Update loop.
        if ep >= config.save_eps and ep % config.update_every == 0 :
            ...
            #when reward is MSE
        #update rl after few episodes and also after few steps
        if ep>=config.int_up and t%config.t_up==0:
            config.t_up = max(config.t_up-10,1)
        # if ep>=int_up:
        if ep>=config.int_up and t%config.t_up == 0 :
            ...
            r_new = (config.r_scale-config.r_scale*(r_new.detach().cpu()/80000.0))
            ...
            if ep <= config.d or t%config.d_int!=0:
        ...
        state_str_t[ep,t+1,5] = env.rtt_policy[-1].item()
    ...
    if ep%500 == 0 and ep > 0:
      path = os.getcwd()
      env_name = 'RTT_based'
      range_folder = 'estimate_rtt/Corrected/lr_3_rtt_alpha_0.125/0.5_1'
      sub_folder = '/Train_'
      dir_save = path + '/' +str(env_name) + '/' + str(range_folder) + str(sub_folder)  + str(ep)
...
#after training
path = os.getcwd()
dir_save = path + '/' +str(env_name) +  '/' + str(range_folder)+  str(sub_folder)   
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
model_DSAC = dir_save + '/Model_DSAC'
model_lstm  =dir_save + '/Model_LSTM'
