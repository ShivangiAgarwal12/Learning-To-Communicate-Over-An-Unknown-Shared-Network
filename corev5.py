'''
use with env-with_q_backlog_avglstm
'''
import numpy as np
import torch
...
class MLPActor(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        self.sp = nn.Softplus()
        self.tan = nn.Tanh()
        # self.fc = nn.Linear(obs_dim-4, 32)
        self.lstm_actor = nn.LSTMCell(3, 3)
        # self.lstm_actor = nn.LSTMCell(3, 32)
        self.relu = nn.ReLU()
    ...
    def forward(self, obs):
        ...
        # hx, cx = self.lstm_actor(out)
        # hx, cx = self.lstm_actor(obs[:,-3:])
        # # out = self.fc(hx)
        # # pdb.set_trace()
        # # out = self.relu(self.fc(hx))
        # # out = self.fc2(out)
        # obs_new = torch.cat((obs[:,0:-3], hx[:]),1)
        # pdb.set_trace()
        # net_out = self.net(obs_new)
...
class MLPQFunction(nn.Module):     
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        # self.lstm_critic = nn.LSTMCell(obs_dim-4, 4)

        self.fc = nn.Linear(32,3)
        self.lstm_critic = nn.LSTMCell(3, 3)
        self.tan = nn.Tanh()
        
    def forward(self, obs):
        # pdb.set_trace()
        # hx , cx = self.lstm_critic(obs[:,4:])
        # obs_new_crit = torch.cat((obs[:,0:4], hx[:]),1)
        # hx, cx = self.lstm_critic(obs[:,-3:])
        # # out = self.fc(hx[:])
        # obs_new_crit = torch.cat((obs[:,0:-3], hx[:]),1)
        # q = self.q(obs_new_crit)
        q = self.q(obs)
        # pdb.set_trace()
