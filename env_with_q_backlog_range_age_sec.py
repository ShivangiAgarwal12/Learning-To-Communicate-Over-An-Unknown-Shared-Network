# -*- coding: utf-8 -*-
"""
control send over network with age in slots and range of 1000
@author: War Horses
"""
#%%import libraries
import numpy as np
...
class Vehicle_model():
   ...
   def env_step(self, agent_AC, k, model, eps_threshold, steps_done):
      ...
      if k == 0:
          ...
          self.age_policy = self.age_policy[:] + 0.1
      else:
          # pdb.set_trace()
...
#         pdb.set_trace()
      ...
      if packet_policy:
          ...
          self.age_policy = 0.1*new_age*np.ones(3)
      ...
      #packet recieved
         ...
         self.age[:] = self.age[:] + 0.1
         self.age = self.age[3:]
         self.age = np.append(self.age, 0.1*new_age*np.ones(3))
         age = torch.tensor([self.age])
         age  = age.to(device)
         age = age.T
