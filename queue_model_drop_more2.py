# -*- coding: utf-8 -*-
"""
updated queue model with dropping packets, if they are not transmitted
in one attempt
"""
#%%import libraries
import numpy as np
...
#         print('Packet in transit',pkt_0)
         ...
         if time_remain == 0:
             return queue, {}, pkt_0, slots
         if time_remain != 0 and time_remain <= config.drop_attempts:
             # pdb.set_trace()
             return queue, {}, {}, slots
         if time_remain > config.drop_attempts:
             # pdb.set_trace()
             #if time remain is not 0, i.e. packet is not received in next slot, make pkt_0 as null and 
             #indicate no packet is in service
             pkt_0 = {}
             time_remain = -1
             packet_service = False
             # pdb.set_trace()
...
#         pdb.set_trace()
         ...
         if packet_service == False:                       
            ...
            if time_remain == 0:
                return queue, pkt_1, pkt_0, slots
            if time_remain != 0 and time_remain <= config.drop_attempts:
                return queue, pkt_1, {}, slots
            if time_remain > config.drop_attempts:
            #only return the receievd packet and nothing for estimator
                # pdb.set_trace()
                time_remain = -1
                #again make packet_0 as null
                pkt_0 = {}
                packet_service = False
                return queue,pkt_1, {}, slots
             ...
             #there is no packet to receive but check whose one slot is left
            # pdb.set_trace()
            if packet_remain == False:
                # pdb.set_trace()
                pkt_1 = deepcopy(pkt_0)
                return queue,{}, pkt_1, -999
            else:
                return queue,{}, {}, -999
   ...
   #no packet arrived  
      ...
      #buffer is not empty 
            ...
            time_remain = transmission(slots)
            if time_remain == 0:
                #this makes sure packet is received in 1 slot
                return queue, {}, pkt_0, slots
            if time_remain != 0 and time_remain <= config.drop_attempts:
                return queue, {}, {}, slots
            if time_remain > config.drop_attempts:
