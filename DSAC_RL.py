...
class ReplayBuffer_nstep:
    ...
    def __init__(self, state_dim, obs_dim, obs_dim_policy, act_dim_policy, log_pi_dim, size, n_step):
        self.mse_input = np.zeros((size, n_step, state_dim), dtype=np.float32)
        self.mse_output = np.zeros((size, n_step, obs_dim), dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
    ...
    def store(self, mse_input, mse_output, lstm_in, lstm_next, obs_policy, next_obs_policy,  act_policy, rew, log_act, done, alpha):
        # pdb.set_trace()
...
class Dsac:
    def __init__(self,env,actor_critic=core.MLPActorCritic, ac_kwargs=dict(), 
            ...
            env_info = {}, n_steps=1, age_hist = 0, lstm_hid=None):
    ...
    def compute_loss_q(self,data):
        ...
        with torch.no_grad():
            ...
            backup = r + self.gamma**(self.n_steps)*Val_next
