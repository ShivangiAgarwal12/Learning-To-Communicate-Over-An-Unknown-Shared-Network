...
class Vehicle_model():
   ...
   def __init__(self, v, px, py, vx, vy, age_hist=0):
      ...
      history_policy = history_packet + self.control_actions + 1
   ...
   def env_step(self, agent_AC, k, model, eps_threshold, steps_done):
      ...
      if k == 0:
           ...
           input_service = torch.cat((self.last_age.unsqueeze(0).T.to(device), \
                                      torch.tensor(self.age_h).unsqueeze(0).T.to(device),\
                                          ...
                                          torch.tensor([[self.avg_p]]).to(device),\
                                          torch.tensor([[self.last_p[-1]]]).to(device)),0)
           # service_est,hx,cx = agent_service(input_service.T.float(),hx_p,cx_p)
           # pdb.set_trace()
           # temp_policy = torch.cat((state_learn.T, torch.tensor(self.age_h).unsqueeze(0).T.to(device),\
           #                          self.last_age.unsqueeze(0).T.to(device),\
           #                              torch.tensor([[self.avg_a]]).to(device),\
      ...
      if packet_policy:
          ...
          self.last_reset_t = (k+1) - self.last_reset_t
          self.reset_age.append(self.last_reset_t)
