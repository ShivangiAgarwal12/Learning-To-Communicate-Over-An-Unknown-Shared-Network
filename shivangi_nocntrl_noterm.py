# -*- coding: utf-8 -*-
"""
Control is received over network in a packet, control only updated when recieved else 
keeps seeing same control history.
@author: War Horses
"""
#%%import libraries
import numpy as np
...
class Vehicle_model():
   ...
   def __init__(self, v, px, py, vx, vy,cx, cy, age_hist = 0):
      ...
      history_policy = history_rep + self.control_actions + 6
   ...
   def check_termination(self,x_curr):
       
       eps_termination = False
       
       if (np.abs(x_curr[0,0] >= 1000)) or (np.abs(x_curr[1,0] >= 1000)) or\
           (np.abs(x_curr[2,0] >= 10)) or (np.abs(x_curr[3,0] >= 10)):
                 eps_termination = True
       
       else:
           eps_termination = False
       return eps_termination
   ...
   def env_step(self, agent_AC, k, model, eps_threshold, steps_done):
      ...
      if k == 0:
           ...
           #                              torch.tensor([[self.age_policy[-1].item()]]).to(device),\
           #                                  torch.tensor([[self.last_p[-1]]]).to(device),\
           #                                  torch.tensor([[self.backlog[-1]]]).to(device),\
